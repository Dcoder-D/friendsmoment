...
const (
	...
	BUCKET_NAME = "friendsmoment-bucket"
)
...
func readFromES(query elastic.Query, index string) (*elastic.SearchResult, error) {
	...
	if err != nil {
		fmt.Printf("Failed to create elastic NewClient", err)
		return nil, err
	}
	...
	if err != nil {
		fmt.Printf("Elastic Search Failed", err)
		return nil, err
	}
...
func saveToGCS(r io.Reader, objectName string) (*storage.ObjectAttrs, error) {
	...
	if _, err := bucket.Attrs(ctx); err != nil { // By checking attrs of bucket, we confirms whether the bucket exisits
		return nil, err
	}
